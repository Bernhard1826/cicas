...
本文的出发点是一个常被忽略的事实：**并非每条带 MUST 的规范都能写成 lint**——许多强义务约束的是 CA 线下行为、证书链或运行时状态，而非单张证书的字节。我们因此把**可 lint 性**（一条规则能否仅凭一张证书的字节被静态裁决）形式化为四个离散 IR 字段上的确定性布尔合取，使判定可复现、误判可追溯到唯一字段；并在其上构建一个从标准文本到合规检查代码的端到端框架，串联规则提取、中间表示（IR）、可 lint 性判定、代码生成与验证。

总设计原则是：仅在确需语言理解之处使用大语言模型（LLM），其余环节实现为可复现的确定性函数。跨文档上下文由规范知识图谱上的确定性子图检索给出；LLM 只承担受 schema 约束的 IR 解析（四元组 ⟨主体, 义务, 谓词, 约束⟩）。代码生成不输出自由 Go 代码，而把代码生成的值域限制为有限原子模板集（79 个原子模板）与类型化词汇表张成的受限 DSL 树空间，在架构层排除"编造字段/OID"类幻觉（命题 1）。针对"代码是否忠实于规范"这一最难判定，本文不依赖 LLM 投票，而构建**证书级语义 oracle**：为每个原子模板生成"满足/违反"两类受控证书、由真实执行读回状态，从而对仅由已认证原子模板构成的生成 lint 给出与模型无关、可复现的"代码≡IR"执行级验证（受控 fixture 上的验证而非对全体证书的定理，详见 §6.5）。缺乏人工真值时，再由**阶段归因式迭代验证（Stage-Attribution Iterative Verification, SAIV）**把召回完整性、代码-规范同义性与双判定源一致性形式化为可计算残差，配以阶段归因与"有下降才接受"的修复策略，在无真值下提供可追溯、可有限终止的质量控制。

在 RFC 5280 与 CABF BR 上的实证如下。关键词召回 2077 条候选，守恒划分得 1555 条真规则、其中 336 条可 lint；zlint 既有实现完整覆盖 132 条，余 204 条为代码生成的直接目标。在这 204 条目标上，"确定性优先、LLM 兜底"两路（均须渲染并通过 `go build`）共生成可编译 lint **175 条（代码生成率 85.8%；确定性 137 + LLM 38）**，其中经去噪多数 LLM 判官判 $\mathrm{Code}\equiv\mathrm{Spec}$ **同义 113 条（同义率 113/175 = 64.6%，分母为能生成可编译 lint 者；not-lintable 规则不计入分母）**。另由证书级 oracle 对其中 124 条独立证得 $\mathrm{Code}\equiv\mathrm{IR}$（确定性、与模型无关的 soundness，非同义判定）。**进一步，把通过同义门的 lint 注入真 zlint 二进制、跑过 1128 张测试证书（§8.5），以三套独立解析器逐条结构审计其每一次命中，发现两条线索：一是同义判官存在漏判——一条被判 EXPRESSES 的 lint 在真证书上实为假阳（丢失"主体非空"前件），印证同义判官的去噪多数表决仍非零误判、且证书级执行是比判官更强的 $\mathrm{Code}\equiv\mathrm{Spec}$ 检验；二是发现 upstream zlint 一处缺陷——其 `e_invalid_ca_certificate_policies` 在证书仅有 `anyPolicy` 一个策略 OID 时返回 `Pass`（注释引用 Table 69），而 CABF BR 7.1.2.2.6 禁止子 CA 出现 `anyPolicy`；我们生成的 `cicasgen_not_any_policy_list_contains_29324` 捕获了 36 条命中（其中 33 条为 zlint 遗漏的 anyPolicy 单独出现），构成 zlint 未覆盖的真缺口。**

**一项贯穿全文的发现是：端到端瓶颈在 Spec→IR 抽取、而非 IR→Code 生成。** 在能生成可编译 lint 的目标上同义率 64.6%，未同义者几乎全部可追溯到上游抽取（主语错抽、把子字段当成整个扩展、前提丢失）；以最强抽取器（gpt-5.4）对这批残差定向重抽实测零增益，说明天花板由抽取任务难度而非模型规模所界。这一归因有确定性支撑：证书级 oracle 对 124 条生成 lint 证得"代码≡IR"，其中 36 条判官仍判不同义——代码忠实于 IR、却不忠实于规范，失效**可证地**落在抽取（$\mathrm{IR}\neq\mathrm{Spec}$）。同义判官比对的是"code_summary≡规范"（code_summary 即机械摘要 $\sigma_{\mathrm{mech}}(t)$、忠实于代码行为），是端到端"代码≡规范"的直接判据、当前仍依赖去噪多数判官——如何把这一最后同义端点进一步确定化，是本框架定位出的首要开放问题。
...
中，关键词召回、可 lint 性判定、渲染、后处理、机械翻译均由确定性函数实现。尤为关键的是，连"生成代码是否忠实于 IR"这一判定本身也被确定化：可认证子集上由证书级语义 oracle 以真实证书执行给出"代码≡IR"的执行级验证，只有 oracle 不适用的忠实性判断和最终"code_summary≡规范"（机械摘要 $\sigma_{\mathrm{mech}}(t)$ 与原文）同义性把关仍需语义灵活的判官（详见 §6.5）。这印证一个更一般的观察——确定性的着力点可由生成端迁移到验证端。缺乏人工真值时，再由 SAIV 将多类质量不变量形式化为可计算残差，提供可追溯的迭代控制信号。

### 1.4 主要贡献

**(C1) 基于 IR 的四条件可 lint 性框架。** 将可 lint 性形式化为四个离散 IR 字段上的确定性布尔合取（道义强度、被约束主体、运行时阶段、规则类型），使判定可复现，任何误判可追溯到唯一 IR 字段而非不透明的端到端调用（§5）。

**(C2) 受限代码空间、可逆机械翻译与证书级语义 oracle。** 将代码生成模块的值域形式化为 79 个原子模板与类型化词汇表张成的受限 DSL 树空间（给出语法、执行语义与词汇封闭性命题 1），并定义确定性机械翻译函数（全函数、确定、原子模板等价意义下可逆）。其上构建证书级语义 oracle：经逐原子模板认证与结构组合，对仅由已认证原子模板构成的生成 lint 给出"代码≡IR"执行级验证，以与模型无关的可复现判据取代单票 LLM 判别器，并诚实披露可认证子集边界（§6）。

**(C3) 阶段归因式迭代验证（SAIV）与多目标残差。** 将召回完整性、代码-规范同义性与双判定源一致性形式化为可计算残差，给出阶段归因规则、修复操作集、有限终止条件与单调性前提；首要修复操作为 IR 内容自反思修复、经确定性闸门自动闭环，并以二元仲裁判官对双判定源一致性残差给出可复算的单调清零路径。该框架在无人工标注真值条件下提供质量控制（§7）。
...
本研究的差异化定位有四。其一，给出贯通"提取 → IR → 可 lint 性判定 → 代码生成 → 验证"的端到端框架，以结构化四元组 IR 为前后两半的统一接缝，而非孤立地做提取或生成。其二，提取侧把 LLM 限定为受 schema 约束的解析器，并以确定性可溯源检索（沿规范结构的显式边遍历，而非向量相似度或 LLM 推断）组装跨文档上下文——差异在于"受约束 + 可溯源"，本文不主张图检索相对普通 RAG 的量化优越性（难做干净消融，见 §9.3）。其三，生成侧将代码生成模块的值域形式化为有限闭合的受限 DSL 树空间，在语言层排除字段/OID 编造类幻觉（命题 1）。其四，验证侧提出阶段归因式迭代验证，将多类质量不变量形式化为可计算残差，在无人工真值下提供可追溯的阶段归因与停止准则——由此给出一项更一般的设计原则：验证链路上每个环节都应尽可能确定化。

## 3. 方法总览

图 1 给出整体结构：系统以 Web PKI 规范文本为输入，输出可编译、可追溯的 zlint Go 检查代码，并由阶段归因式迭代验证（SAIV）在无人工真值下闭环修复。六阶段分两半——前半（确定性上下文、提取、判定）构成"规范 → 可 lint 规则"，后半（合成、对齐、验证）构成"可 lint 规则 → 可信代码"，以结构化中间表示 IR 与四条件可 lint 性判定为统一接缝。
...
本文不让 LLM 直接判定，而把它分解为四个独立布尔条件、各只读一个 IR 字段、全满足才判可 lint，从而在 IR 上确定性计算可 lint 性。三点观察：其一，决定可 lint 性的恰是四类信息——义务道义强度、被约束主体（含数据范围：单证书 vs 跨证书/其他制品）、约束生效阶段、规则类型；其二，四者各可编码为取值于小而封闭集合的单一 IR 字段，故判定退化为四个离散字段的布尔函数；其三，分离使任何错误标签可追溯到恰好一个 IR 字段、而非不透明的端到端调用。
...
- **$C_2$（主体边界 / 单一证书）**：$C_2(r)$ 由两项联合判定：其一，$r.\text{assertion\_subject} = \text{Certificate}$；其二，$r.\text{subject}$ 路径的根段（第一个路径分量）属于预定义的证书/CRL 结构字段集合。仅 $\text{assertion\_subject} = \text{Certificate}$ 纳入判定——$\text{CA}$ 不在范围内（CA 是颁发证书的机构，其行为不在证书字节中）；$\text{RelyingParty}$ / $\text{Implementation}$ 描述依赖方或软件行为；$\text{CRL}$ 描述证书吊销列表文档（不在本文范围内）；$\text{CrossArtifact}$ 描述需跨制品比对（如"子证书的 SKI 须等于签发者的 AKI"、预证书须与最终证书逐字节一致、序列号跨证书唯一等）。第二项 subject 路径门确保：若 subject 路径指向操作名词（如 domain_validation_record、phone_contact）而非证书字段，该规则描述的是 CA 过程而非证书内容，同样不可 lint。
...
§6.1 限定了树的*结构*，却没约束原子模板的*参数*——参数若可任填，LLM 仍能写出引用虚构字段或 OID 的"结构合法"之树。本节用一张**类型化词汇表** $\mathcal{V}$ 封住这道缺口：$\mathcal{V}$ 是七类有限集合的不相交并、在系统启动时冻结，每个原子模板参数只能取自其中某一类——证书字段名、DN 字段、OID 常量、KeyUsage 位、ExtKeyUsage 位、ASN.1 编码类型、命名正则（各类规模与样例见附录 D）。
...
- **可逆性**：由 $\sigma_{\mathrm{mech}}(t)$ 可机械还原 $t$（同义原子模板映射同一短语时不可区分），故 $\sigma_{\mathrm{mech}}$ 不构成信息瓶颈（命题 2）。
...
**受限 LLM 合成。** 当确定性合成返回 $\perp$（不可归约）时，生成器转入 LLM 合成——模型拿到规则上下文、结构化 IR、由 $\mu$ 收窄的候选原子模板集，以及 $\mathcal{V}$、$\mathcal{A}$ 的可读枚举与签名表（实际提示的四区段拼接见附录 G），只输出两种结果之一：一棵序列化的 DSL 树，或一个显式的"无模板"弃权标记 $\perp_{\mathrm{NT}}$。后者当且仅当模型判断当前原子模板集与词汇表不足以表达该规则时返回，使该规则转入修复路径而非被勉强渲染成"形式合法但语义错位"的代码。无论走哪一支，凡进入渲染的树都落在 $\mathcal{T}_{\mathcal{V}}$ 内。
...
**层次 B：代码行为摘要的语义对齐。** 这是核心层，把"代码是否实现规范"这一跨模态判定，归约为"两句自然语言是否同义"。做法是：由摘要函数 $\sigma$（系统持有 DSL 树时即其确定化实现 $\sigma_{\mathrm{mech}}$，见 §6.3 与附录 E）把代码翻译成一句"该代码检查什么约束"的摘要，再判它与 `Description` 是否同义、给出置信度 $c_{\mathrm{syn}}$。
...
**证书级语义 oracle（$\mathrm{Code}\equiv\mathrm{IR}$ 的执行级验证，独立于同义判定）。** 为每个原子模板造一对受控 fixture（谓词真/假，按原子模板类参数化、不绑定具体规则），令代码真执行、读回状态，两张都符期望即认证该原子模板。对持 DSL 树 $t$ 的自生成 lint：当 $t$ 仅由已认证原子模板构成且编译通过时，$\rho(t)$ 逐原子忠实，由结构归纳得 $\mathrm{Code}\equiv t$；又因 $t$ 忠实归约自 IR，故 $\mathrm{Code}\equiv\mathrm{IR}$。这是受控 fixture 上的执行级验证，确定、不随模型而变。**关键限定：$\mathrm{Code}\equiv\mathrm{IR}$ 不是同义性。** oracle 的价值在另一处：当它认证 $\mathrm{Code}\equiv\mathrm{IR}$、判官却判 $\mathrm{Code}\not\equiv\mathrm{Spec}$ 时，由传递链可**确定性地把该失效定位到抽取**（$\mathrm{IR}\neq\mathrm{Spec}$、代码无辜），从而以可证的方式坐实"瓶颈在抽取"（§8.3 给出该子集计数）。
...
其中 $\mathrm{alias}(\cdot)$ 是词干化加一张冻结的标准扩展别名表。判读分 $\mathtt{ENTITY\_OK}$ / $\mathtt{ENTITY\_MISMATCH}$（检查了文本未提及的实体，可疑）/ $\mathtt{NO\_ENTITY}$（不引用实体，不适用）三种。它专捕"分段或指代把错误主语交给生成"这一失效，与层 B 分歧处即定位主语错抽；盲区（指代、别名缺口）只会保守地多报 $\mathtt{ENTITY\_MISMATCH}$。
...
第 6 节的方法在单次前向生成下已能产出结构完整的 zlint 代码，但在大规模、无人工真值的场景下仍有质量风险，且风险高度集中在最上游的 NL$\to$IR 提取：IR 内容抽错会使下游分类、生成、验证全作用在错误对象上，而召回数量由结构不变量 G1（§7.3）独立闭合。故首要着力点是 IR 内容，其修复操作 $\rho_R$ 排在修复序列最前。SAIV 沿管道反向定位误差阶段再定向修复，把若干可直接计算的不变量当作损失信号；这些信号由结构性不变量与客观外部证据支撑，使质量验证由可计算残差而非人工真值承担。

### 7.1 形式化定义

受控规范-代码生成管道 $\Pi$ 是四个模块依次串联的复合函数——**召回 $\to$ 分类 $\to$ 生成 $\to$ 验证**：召回模块 $\phi_R$（RFC 2119 关键词驱动）找出候选规则，分类模块 $\psi_C$（§5 的四条件可 lint 性判定）筛出可执行规则，生成模块 $\phi_G$（受限于 §6 的 DSL 代码空间 $\mathcal{T}_{\mathcal{V}}$）合成检查代码，验证模块 $\phi_V$（§6 的三层语义对齐）把关。SAIV 即沿这条管道反向定位误差阶段、再定向修复。

### 7.2 三个迭代目标（优化标签）

SAIV 借鉴反向传播的思路：先定下整个流程要优化的目标，再沿管道反向定位是哪一阶段拖累了目标、并定向修复。本节先给出这三个目标，后续各节（残差、归因、算法）都围绕逼近它们展开。
...
- **G2（未覆盖规则的代码—规范同义）**：管生成的代码是否如实表达规范。盯未覆盖子集 $\mathcal{R}_L^{\mathrm{uncov}}$，要求代码摘要 $\sigma(\phi_G(r))$ 与规范原文同义（§6.5 传递链）。**同义性对全部 Form A 由层 B 判官判定**（$\mathrm{Code}\equiv\mathrm{Spec}$），证书级 oracle 的 $\mathrm{Code}\equiv\mathrm{IR}$ 是正交的确定性 soundness 信号、不计入同义分子；二者分歧处把失效确定性归因到抽取（§8.3）。残差为代码忠实残差 $\mathcal{L}_{\mathrm{code}}$。自动生成的主要价值在补足生态空白，故未覆盖子集是 G2 最重要的观测域。
...
| G2 同义性 | $\mathcal{L}_{\mathrm{code}}$（式 7） | **$\rho_R$**；$\rho_G$ | IR 内容修正 / DSL 树重合成 / $\sigma_{\mathrm{mech}}$ 替换 |
...
设 $r \in \mathcal{R}_L$，$c = \phi_G(r)$。由于 $c$ 的 `Description` 已由确定性后处理与规范原文对齐（§6.6），代码正确性标签 $\lambda_{\mathrm{code}}(r) \in [0,1]$ 由"编译 × 结构得分 × 同义置信度"之积度量；由 §6.5 的语义等价传递链，$\sigma(c)\equiv\mathrm{spec}(r)$ 且 `Description`$\equiv\mathrm{spec}(r)$ 即可推出 $c\equiv\mathrm{spec}(r)$。同义置信度 $c_{\mathrm{syn}}$ 一律由判官给出（$\mathrm{Code}\equiv\mathrm{Spec}$）；证书级 oracle 的 $\mathrm{Code}\equiv\mathrm{IR}$ 不替代该项，而是与之并置——二者分歧（$\mathrm{Code}\equiv\mathrm{IR}$ 成立但 $c_{\mathrm{syn}}$ 低）即把残差确定性地归因到抽取（$\mathrm{IR}\neq\mathrm{Spec}$）。
...
**阶段归因。** 每轮用两个可直接计算的量定位出错阶段：可 lint 集上的编译失败率与平均结构得分。四分支互斥完备、自上游向下游依次判（默认阈值 $\tau_R=\tau_C=0.10$）：召回守恒残差超阈，归**召回/分类**阶段；否则编译失败率超阈，归**分类**误判；否则编译通过但结构得分 $<1$、对齐低，归**生成**阶段；否则结构完整、编译通过而同义率低，归**验证**阶段。G1 闭合后（召回守恒残差 $\approx 0$，§8.2），召回分支的误差不再是召回数量、而是 IR 内容（主语/极性/约束/前提抽错），故其修复重定向为下文的 IR 内容修复 $\rho_R$。无人工真值时的触发信号是两个确定性比例：可归约 lint 中被实体筛查判 $\mathtt{ENTITY\_MISMATCH}$（非指代）者、与持续弃权 $\perp_{\mathrm{NT}}$ 者；偏高即触发 $\rho_R$。

**修复操作。** IR 内容修复 $\rho_R$ 是首要操作、也是唯一改动 IR 本身的操作：上游 NL$\to$IR 一旦出错（主语错抽、极性反转、约束散文化、前提丢失），下游分类、生成、验证都失去正确对象。其余三个操作——分类修复、生成修复、验证修复——并列、无优先级，由归因决定本轮触发哪个，均在 IR 给定下工作。

- **IR 内容修复 $\rho_R$**：当生成在 $K_{\mathrm{loc}}$ 轮内持续弃权、或同义性持续低于阈值时，把下游完整失败轨迹——当前 IR、不可归约类别、机械摘要、判官裁定与理由、同义置信度、本会话已试过的历史 IR——构成一条**反向失败信号**回传 LLM，令其自我诊断并决定：**(a)** 更正 IR 的主语 / 谓词 / 约束 / 前提（连同取自原文、支持该修正的证据子串一并输出），或 **(b)** 声明 `NO_FIX`——确认 IR 已正确而当前词汇确不足以表达该规则。它不是盲目重试（同提示 → 同错），也不是静态规则修补，而是一次能看到完整下游证据的反思调用，修复管道最上游的 NL$\to$IR 阶段。
...
```
算法 2：阶段归因式迭代验证（SAIV），ρ_R 为首要修复操作、g4 自动闸门负责回路内验收
...
输出：最终代码集 C* 与终止状态 status
 ...
 7:      stage ← StageAttribution(...)                    // 式(9)；G1 已闭合 ⇒ 首查 IR 内容
 8:      if stage = φ_R then                              // 首要修复操作 ρ_R：IR 内容自反思修复
...
| **可 lint（单证书可观测）** | **336** | 经硬化分类器：负向门排除过程/跨证书/runtime/语义类，并对 13 条误标可 lint 的规则（跨证书/线下验证/外部语义/MAY 许可）在抽取源头降级；CRL 文档规则不在本口径 |
...
| 不同义（DNE） | **62** | 详见下文 |
| *（旁证）证书级 oracle $\mathrm{Code}\equiv\mathrm{IR}$* | *124* | *确定性 soundness、非同义；其中 36 条判官判 DNE，确定性地把失效归因到抽取（§8.3）* |

**两类残差的逐条归因——不可生成可编译 lint（29 条）与不同义（62 条）——按失败环节确定性复算。** 其要点：不可生成的过半为上游抽取问题（约束值丢失/主语未解析）、其余为代码生成能力边界（编码语义/字节级 DER/真残差）；不同义的绝大多数仍可追溯到上游抽取（约束被降级为存在性、适用前提与例外漏抽），少数为代码派发缺口（原子可达但路由到粗检查、工程可修）与真能力边界（编码语义、格式转换）。尤其，36 条经证书级 oracle 证得 $\mathrm{Code}\equiv\mathrm{IR}$ 却被判官判 $\mathrm{Code}\not\equiv\mathrm{Spec}$ 的规则，**确定性地坐实失效出在抽取（$\mathrm{IR}\neq\mathrm{Spec}$）而非生成**。两者共同印证全文主结论——**端到端瓶颈在 Spec→IR 抽取，而非 IR→Code 生成**；"可 lint 却生不出/生成却不同义"皆非可 lint 性定义的反例，而是抽取质量或当前原子覆盖的边界。
...
一条可 lint 规则能否被写成同义 lint，按口径分层报告（完整漏斗见 §8.1 表 1）：codegen 定义域 = **204 条**未覆盖目标；生成器以"确定性优先、LLM 兜底"两路合成，两路均须渲染并通过 `go build` 才计入——**能生成可编译 lint（Form A）175 条（确定性 137 + LLM 38），代码生成率 175/204 = 85.8%**；其上由去噪 5 票 LLM 判官判 $\mathrm{Code}\equiv\mathrm{Spec}$，得 EXPRESSES **113 条**，**同义率 = 113/175 = 64.6%**（分母为能生成可编译 lint 者）。两路的同义贡献悬殊：确定性树 105/137 = 76.6% 同义，而 LLM 兜底树 8/38 = 21.1%——LLM 把生成覆盖从 137 抬到 175，但补进来的多是确定性路啃不动的硬规则，同义率仍显著偏低。独立地，证书级 oracle 对 124/175 条证得 $\mathrm{Code}\equiv\mathrm{IR}$（确定性 soundness，非同义）；其中 36 条判官判不同义，正是把失效确定性归因到抽取的可证据点。

同义率全程由判官按 $\mathrm{Code}\equiv\mathrm{Spec}$ 判定（去噪 5 票多数表决以抑制单票方差），不因 oracle 认证而短路——这是相较"把 $\mathrm{Code}\equiv\mathrm{IR}$ 当作同义"更保守、更诚实的口径。判官信号上的方法学加固（reducer 极性修复、obligation-aware 渲染、profile-scope 判定、定向重抽）见上文。同义率 64.6% 仍暴露了真问题：确定性树同义率达 76.6%，而 LLM 兜底在确定性啃不动的硬规则上仅 21.1% 同义，端到端瓶颈集中在上游抽取（见 §8.3 的 oracle 归因）。

### 8.4 lintability外部验证

引入 zlint 维护者公开的 CABF BR 映射表 [26] 作为完全独立的第三方人工金标，覆盖 BR 1.4.8 和 BR 2.0.2 两个版本，分别与本文按相同双层流水线提取的 CABF-Server-1.4.8 后端（422 条）、CABF-Server-2.0.2 后端 [27]（1024 条）进行同版本对照。
...
### 8.5 证书检测作为 SAIV 的一道门

前述同义发射只证 $\mathrm{Code}\equiv\mathrm{Spec}$；要确认生成的 lint 在**真证书**上行为正确，须把它注入真 zlint 二进制并跑过证书语料。这一步本身就是 SAIV 的一道门：一条 `cicasgen_` lint 命中一张证书，要么是 upstream zlint 漏掉的**真问题**（new lint），要么是**假阳**——而假阳即反推某上游阶段（IR 抽取 / 可 lint 性 / 代码生成 / 同义）出错并定位修复。
...
**结果（表 IV）。** 扫描 1128 证书得 **108** 条 `cicasgen_` 命中。

**TABLE IV：证书检测 SAIV 门（31 lint / 1128 testdata 证书）**

| triage 判定 | 数 | 反查后 |
|---|---:|---|
| REAL（upstream 共识 / known-bad fixture） | 91 | — |
| **SPURIOUS（假阳）** | **0** | — |
| UNCERTAIN（无 oracle 信号、窄发射） | 17 | 17 条经 `openssl` 反查全部 **CONFIRMED_REAL** |
| **合计** | **108** | **108/108 真检测，0 假阳** |
| *独立结构审计* | | *108/108 CONFIRMED，0 REFUTED/NOCHECK；每条均由 ≥2 套独立解析器一致确认* |

两类检测是 upstream zlint **未覆盖**的真覆盖缺口。其一为 `cicasgen_not_any_policy_list_contains_29324`（CABF BR 7.1.2.2.6）：**36** 条命中全部位于子 CA（CA:TRUE 且 subject≠issuer）证书，`openssl` 反查确认每张确含 `anyPolicy`。zlint 虽有 `e_invalid_ca_certificate_policies` 试图覆盖该条款，但其实现有一个缺陷：当证书仅有 `anyPolicy` 一个策略 OID 时（33 条命中），它返回 `Pass`（注释引用 Table 69），而 CABF BR 明确要求子 CA 证书不得出现 `anyPolicy`；zlint 仅在 `anyPolicy` 与其他 OID 共存时才报错。其二为 `legalChar.pem` 上的 `cicasgen_when_oid_policy_organization_validated_list_contains_29475`——OV 证书（策略 `2.23.140.1.2.2`）携带 `givenName`（CABF BR 7.1.2.7.4 禁止），upstream zlint 无任何对应 lint。
...
本文结论严格受限于所用输入表示（结构化 IR）、受限代码空间与验证流程，更适合可在单文档上下文闭合的静态约束，不应外推为"所有 PKI 规范均可完全自动化"。六类边界：**(i) 方法表达力**——同义判定端点仍由 LLM 实现，79 原子模板对字节级编码、宿主未暴露字段、动态约束等仍有缺口；**(ii) 组件必要性未消融**——知识图谱/受限 DSL/SAIV 的支持分别为工程取舍、架构论证与未消融项，本文不主张图检索相对普通 RAG 的量化优越性；**(iii) 现代 LLM 基线缺位**——直接生成 Go 的对比未做，但证书级 oracle 生成器无关，使其成为定义明确的未来工作；**(iv) 覆盖与泛化边界**——可 lint 总量 336 与 codegen 目标 204 不同口径，跨体系泛化本文仅给机制论证；**(v) 端到端口径**——"代码≡规范"仅对发射集成立（§8.3），可 lint 标签未对全部 336 条穷尽人工审计；**(vi) 同义端点盲区**——code_summary≡规范 是比对 code_summary 与原文的 LLM 判官、非 oracle 那种与模型无关的检查，且其"原文"取自抽取时逐字存下的 rule_text（IR 内字段）而非重读规范源——故 Layer-1 若把句子截断或漏抽从句，rule_text 与据其解析的 IR 会一致地错、同义判官无从察觉（与"瓶颈在抽取"互为印证）。此外生成 lint 以 `cicasgen_*` 前缀与 zlint 两端互拒以防"自己覆盖自己"。

## 10. 结论

本研究面向 PKI"规范-到-合规检查代码"的端到端自动化，给出贯通"提取 → IR → 可 lint 性判定 → 代码生成 → 验证"的统一框架。实证支持四点结论与一项方法学论断。其一，Web PKI 规范与现有 lint 工具存在显著覆盖缺口：336 条可 lint 中 zlint 完整覆盖 132，余 204 是自动生成的直接目标。其二，可靠提取依赖受约束提取而非端到端直接提示——把 LLM 限为 schema 受限解析器、以确定性检索与确定性可 lint 性判定包裹。其三，真规则中仅约 22%（1555 中 336）可还原为单证书静态 lint，这是对"哪些规范可被静态强制"的量化回答。其四（负向发现）：生成端瓶颈在抽取，未同义者几乎全部可追溯到上游 IR 欠表达，以最强抽取器定向重抽残差实测零增益。方法学论断：验证链路每个环节都应尽可能确定化（最有力实例即证书级 oracle 把忠实性判定确定化），并以"可归约子集闭合 + 不可归约边界披露"取代单一收敛阈值或单一同义率指标。
...
$\mathcal{A}$ 的 79 个原子模板中 **62 个 GENERIC、17 个 NON_GENERIC**。NON_GENERIC 的 17 个为：`CommonNameFromSAN`、`SigAlgMatchesTBSSignature`、`NotAfterIsNoExpirySentinel`、`DomainComponentOrdered`、`DNDirectoryStringValuesEncodedAs`、`CertPolicyExplicitTextHasEncodingTagInSet`、`CRLDPHasNameRelative`(+`WithMultiIssuer`)、`AIAHasMethodOtherThan`、`AIAMethodLocationsAnyMatchRegex`、`AIAMethodLocationsTagInSet`、`IPv4Conditional`、`SubtreeIPv4Conditional`、`SubtreeIPListAnyAllZero`、`SubtreeIPListAnyHasOctetCountAndNotAllZero`、`SubtreeIPMaskValidCIDR`、`WildcardFilter`。允许 NON_GENERIC 原子模板进入代码生成（它们对应真实的 PKI 构造，非语料注水），但要求显式标记并诚实披露其占比：**113 条同义发射 lint 中，绝大多数完全由 GENERIC 原子模板构成，仅少数依赖至少一个 NON_GENERIC 原子模板**。这一边界既说明框架主体由通用原语支撑，也不掩盖少数特化构造的存在。
...
每次调用的完整 prompt 由四区段依次拼接：**区段 A（Rule Context）**——源 ID、章节号、规则 ID、逐字 `rule_text` 与 `source` 元数据；**区段 B（Structured IR）**——扁平展开的 IR 四/五元组；**区段 C（DSL Schema）**——附录 D 所述 $\mathcal{V}$ 与 $\mathcal{A}$ 的全量枚举（按分量分组的字段名清单与按语义簇分组的原子模板签名表，附每原子模板一行 PKI 语义注释）；**区段 D（Output Protocol）**——两种合法 JSON 形态的精确 schema 与一条 minimal positive 示例。

### 附录 G：SAIV 残差与归因的形式化细节
