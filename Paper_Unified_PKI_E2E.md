...
本文提出一个从标准文本到合规检查代码的端到端框架，串联规则提取、中间表示（IR）、可 lint 性判定、代码生成与验证五个环节，其设计原则是仅在确需语言理解之处使用大语言模型（LLM）、其余环节尽量实现为可复现的确定性函数：(1) 跨文档上下文由 PKI 规范知识图谱上的确定性子图检索给出，LLM 仅承担受 schema 约束的语义解析、将规则转写为结构化 IR（四元组 ⟨主体, 义务, 谓词, 约束⟩）；(2) 可 lint 性形式化为五个离散 IR 字段上的确定性布尔函数 $C_1\wedge C_2\wedge C_3\wedge C_4\wedge\neg C_5$，误判可追溯到唯一字段；(3) 代码生成不输出自由 Go 代码，而把 $\phi_G$ 的值域限制为原子集 $\mathcal{A}$（$|\mathcal{A}|=68$）与类型化词汇表 $\mathcal{V}$ 张成的受限 DSL 树空间 $\mathcal{T}_{\mathcal{V}}$，在架构层排除"编造字段/OID"类幻觉（命题 1）；(4) 针对"代码是否忠实于规范"这一最难判定，不依赖 LLM 投票，而构建证书级语义 oracle——为每个原子生成"满足/违反"两类受控证书、由真实执行读回状态，从而对仅由已认证原子构成的生成 lint 给出与模型无关、可复现的 $\mathrm{Code}\equiv\mathrm{IR}$ 执行级验证（受控 fixture 上的验证而非对全体证书的定理，详见 §6.7）。

针对大规模代码生成缺乏人工真值这一困难，本文提出阶段归因式迭代验证（Stage-Attribution Iterative Verification, SAIV）框架，将召回完整性（G1）、lint 覆盖度（Gcov，相对 zlint 既有实现的覆盖广度，分母为 zlint 实测合计 276（纯证书 lint））、代码-规范同义性（G2）与双判定源一致性（G3）形式化为可计算残差，配以阶段归因规则与单调修复算子，在无人工真值下提供可收敛、可追溯的质量控制。其首要修复算子 $\rho_R$（IR 内容自反思修复）经确定性 g4-sanity 闸门全自动运行，把"发现 IR 抽错并自动重抽修正"纳入迭代回路。

在 RFC 5280 与 CABF BR 上的实证：关键词召回 2077 条候选，守恒划分得 1601 条真规则，其中 389 条可 lint；zlint 既有实现完整覆盖 148 条、部分覆盖 31 条，余 241 条为代码生成的直接目标。对全部可 lint 规则，确定性归约即得 254 条满足 $\mathrm{Code}\equiv\mathrm{IR}$ 的 DSL 树（证书级 oracle 与模型无关地验证，190 条编译通过）；经 $\mathrm{Code}\equiv\mathrm{IR}$（oracle）与 $\mathrm{IR}\equiv\mathrm{Spec}$（去噪 5 票同义判官）双门后发射 83 条 lint，发射集构造性 100% 同义。未筛选时最难子集原始同义率约 24–27%，其不同义几乎全部可追溯到上游抽取（主语错抽、子字段当成整个扩展）而非代码生成——由此定位出一项发现：瓶颈在 $\mathrm{Spec}\to\mathrm{IR}$ 抽取，而非 $\mathrm{IR}\to\mathrm{Code}$ 生成。其中 $\mathrm{Code}\equiv\mathrm{IR}$ 为自动验证、$\mathrm{IR}\equiv\mathrm{Spec}$ 当前由人工把关，故端到端 $\mathrm{Code}\equiv\mathrm{Spec}$ 仅对发射集成立，其自动化是本框架定位出的首要开放问题。
...
**第二，生成侧——代码空间应在语言层面而非 prompt 层面受限。** 与其用 prompt 反复约束 LLM 不要编造字段，不如使其无从编造：把生成算子 $\phi_G$ 的值域定义为有限闭合的 DSL 树空间 $\mathcal{T}_{\mathcal{V}}$，使编造字段/OID 的幻觉在架构层即被排除（命题 1），不依赖 prompt 约束或 few-shot 引导这类概率性手段。
...
中，$\phi_R$、渲染 $\rho$、后处理 $\Phi_{\mathrm{post}}$、机械翻译 $\sigma_{\mathrm{mech}}$ 均由确定性函数实现；尤为关键的是连"生成代码是否忠实于规范"这一判定本身也被确定化——可认证子集上由证书级语义 oracle 以真实证书执行给出 $\mathrm{Code}\equiv\mathrm{IR}$ 的执行级验证，仅在 oracle 不适用时才退回 $\phi_C$、$\phi_V$ 等需要语义灵活性的 LLM 端点（详见 §6.7）。这印证一个更一般的观察：确定性的着力点可由生成端迁移到验证端。缺乏人工真值时，再由阶段归因式迭代验证（SAIV）将多类质量不变量形式化为可计算残差，提供收敛性与可追溯性。
...
**(C2) 受限代码空间、可逆机械翻译与证书级语义 oracle。** 将 $\phi_G$ 的值域形式化为原子集 $\mathcal{A}$（$|\mathcal{A}|=68$）与类型化词汇表 $\mathcal{V}$ 张成的 DSL 树空间 $\mathcal{T}_{\mathcal{V}}$（语法、执行语义、词汇封闭性命题 1），并定义确定性机械翻译算子 $\sigma_{\mathrm{mech}}$（全函数、确定、原子等价意义下可逆）。其上构建证书级语义 oracle：经逐原子认证与结构组合，对仅由已认证原子构成的生成 lint 给出 $\mathrm{Code}\equiv\mathrm{IR}$ 执行级验证，以与模型无关的可复现判据取代单票 LLM 判别器，并诚实披露可认证子集边界（§6）。

**(C3) 阶段归因式迭代验证（SAIV）与多目标残差。** 将召回完整性（G1）、lint 覆盖度（Gcov，分母为 zlint 实测合计 276（纯证书 lint））、代码-规范同义性（G2）与双判定源一致性（G3）形式化为可计算残差，给出阶段归因规则、修复算子谱与收敛性论证；首要算子 $\rho_R$（IR 内容自反思修复）经确定性 g4-sanity 闸门全自动运行，L4b 二元判官使 G3 残差单调降至 0。该框架在无人工标注真值条件下提供质量控制（§7）。
...
并非每条带 MUST 的规范都能写成 lint 检查。有的 MUST 约束的是 CA 的线下行为（如"CA 必须核验申请人身份"），有的要比对证书链上的其他证书，有的要查 DNS 记录或撤销历史——这些都无法仅凭一张证书的字节静态裁决。因此在生成代码之前，必须先回答一个前置问题，即每条规范规则的**可 lint 性**：它能否被一个不依赖外部上下文或运行时行为、仅凭**一张证书**的字节即可裁决的静态检查所表达？（CRL 等其他制品自身的合规检查不在本文范围内。）
...
- **可逆性（原子等价意义下）**：由 $\sigma_{\mathrm{mech}}(t)$ 可机械还原 $t$（同义原子映射同一短语时不可区分），故 $\sigma_{\mathrm{mech}}$ 不构成信息瓶颈（命题 2，§7.9）。
...
为把第 6 节的算子串成一条可追踪的链，以下用一条真实规则走完整个流程。以 RFC 5280 §4.2.1.9 关于 dNSName 编码的规则为例。规范原文："Conforming implementations MUST convert internationalized domain names to the ASCII Compatible Encoding (ACE) format ... before storage in the dNSName field." 经受控提取得到 IR：
...
默认 $w_R = w_C = 0.5$。定理 1 是理想恒等，$\mathcal{L}_{\mathrm{recall}}$ 度量其在实际运行（$\phi_C$ 由 LLM 辅助、可能漏判或幻觉）中被违反的程度；对称归一化同时惩罚两个方向（丢规则 / 增条目）。

**覆盖度目标（lint 覆盖残差 $\mathcal{L}_{\mathrm{cov}}$）。** 上述两项损失刻画"召回是否守恒"（G1）与"可 lint 规则是否被忠实写成代码"（G2），但**未**刻画一个同样关键的工程目标：本系统在**非硬编码**前提下，究竟能覆盖多少现有静态检查工具已实现的规则。以 zlint 为参照系——其在目标标准族的 lint 数是可直接查得的客观基准（项目内置 zlint 源码的**证书 lint**：RFC 5280 共 **122** 条、CABF BR 共 **154** 条，合计 **276** 条；CRL lint（RFC 9 + CABF 16，计 25 条）因 CRL 文档自身的检查不在本文单证书可 lint 口径内，不计入分母）——定义覆盖残差
...
分母 $|\mathcal{C}_{\mathrm{zlint}}|$ 取 zlint 在该标准族的**证书 lint** 总数（122/154，合计 276；CRL lint 不计入，与本文单证书可 lint 口径一致）。$\mathcal{L}_{\mathrm{cov}}\to 0$ 即**在不硬编码任一 lint 前提下，端到端生成的覆盖广度逼近 zlint**；"非硬编码"由 §9.4 的 `cicasgen_*` 防火墙保证（生成 lint 与 zlint 原生两端互拒）。该残差与 G2 正交：G2 管"已写的对不对"、$\mathcal{L}_{\mathrm{cov}}$ 管"该写的还有多少没写"，后者由 $\rho_R$ 与离线扩原子 $\rho_A$ 驱动下降。当前远未闭合（见 §8.1a/§8.3）。
...
四分支互斥完备（默认 $\tau_R=\tau_C=0.10$）：结构不变量违反 → 回 $\phi_R$；否则编译失败率超阈 → $\phi_C$ 误分类；否则编译通过但对齐低且结构得分 $<1$ → $\phi_G$；否则结构完整、编译通过而同义率低 → $\phi_V$ 判定偏差。
...
机械翻译算子 $\sigma_{\mathrm{mech}}$ 已在 §6.4 引入并列出其类型安全、决定性、可逆性三项性质（可逆性即下文命题 2）；本小节补足其构造与动机。§6.7 把通用摘要算子 $\sigma$ 实现为 LLM 调用，但当输入是可结构化拆解的 DSL 树时，LLM-$\sigma$ 会引入一类可消除的系统性失真。
...
**关键性质。** 除 §6.4 已列的全函数性、确定性、可逆性外，$\sigma_{\mathrm{mech}}$ 还具**极性正确性**：条件前提为否定时显式生成 "WHEN NOT" 形式，消除上述 LLM-$\sigma$ 的 NA-反转失真。
...
**对验证链路的意义。** 引入后，传递链 $\mathrm{Spec} \to \mathrm{IR} \to t \xrightarrow{\sigma_{\mathrm{mech}}} \mathrm{Summary} \equiv \mathrm{Description}$ 中 LLM 仅余 $\phi_C$、$\phi_V$ 两端，概率失真被压缩至**原子词典 $\mathcal{M}$ 的离线选择**、不再随每条规则采样。$\sigma_{\mathrm{mech}}$ 仅适用于本系统自生成（持有 DSL 树）的 lint；外部既有 lint 仍需 LLM-$\sigma$。

### 7.10 不变量残差 L4b 修复

前文 $\rho_C$（§7.7）用"实现存在性 $\Rightarrow$ 可执行性"的单向推论。本节扩为双向 falsifiable 残差：给定外部工具覆盖 $\mathrm{cov}_{\mathcal{T}} : r \mapsto \{\text{full}, \text{partial}, \text{none}\}$，定义违反集 $\mathcal{V} = \{ r : \mathrm{cov}_{\mathcal{T}}(r) \in \{\text{full}, \text{partial}\} \land \phi_C(r) \neq \mathrm{lintable} \}$，$N_{\mathrm{viol}} = |\mathcal{V}|$；$\phi_C$ 与 $\mathrm{cov}_{\mathcal{T}}$ 同时正确则 $N_{\mathrm{viol}} = 0$，故它是不依赖人工真值的可计算残差。
...
L4b 是 §7.7 中 $\rho_V$ 的特化：不改代码或生成模块，只修双判定源的不一致；触发 $N_{\mathrm{viol}} > 0$、终止 $N_{\mathrm{viol}} = 0$，使该 falsifiable 残差具备真正的"零点"、可作收敛判据。
...
**其一，把忠实性判定本身确定化。** 传统验证靠人工（不可规模化）或 LLM 投票（不可复现）；本文以证书级 oracle 用真实执行取代投票、给出与模型无关的 $\mathrm{Code}\equiv\mathrm{IR}$（§8.4）。同一思路贯穿全链路（受限 DSL 排除字段/OID 编造、$\sigma_{\mathrm{mech}}$ 消除摘要极性失真），提炼出"验证链路每个算子都应尽可能确定化、着力点可由生成端迁移到验证端"。

**其二，同义性应作发射判据而非事后指标。** 未覆盖子集原始同义率约 24–27%（§8.5）不应读作"四分之三代码错"——几乎全源于上游 IR 抽取，归约器忠实地把错误 IR 映成错误树。正确姿态是只发射经 $\mathrm{Code}\equiv\mathrm{IR}$ 与 $\mathrm{IR}\equiv\mathrm{Spec}$ 双验证的 lint，使发射集构造性 100% 同义，"收敛"重定义为发射集在保持 100% 下持续增长。
...
**现代 LLM 基线的缺位。** 让前沿模型直接生成 Go lint 效果如何，是当前最重要的实证缺口；本文未做此对比。但证书级 oracle 是生成器无关的——它对任意来源（含直接生成、含不同前沿模型）的 lint 都能以同一执行级判据验证 $\mathrm{Code}\equiv\mathrm{IR}$，故"在同一 254/389 目标集上评测直接生成基线的通过率"是定义明确的未来工作，而非方法学障碍；其结论将直接检验"生成不是瓶颈、抽取才是"这一观察。
...
**端到端正确性口径。** 两条腿：$\mathrm{Code}\equiv\mathrm{IR}$ 由证书级 oracle 自动验证、$\mathrm{IR}\equiv\mathrm{Spec}$ 当前由人工同义裁定把关，故端到端 $\mathrm{Code}\equiv\mathrm{Spec}$ 仅对发射集成立（§8.5）；本文不声称已自动求解 $\mathrm{Spec}\to\mathrm{IR}$，反把"端到端自动化"的未决核心定位到此上游阶段。389 的可 lint 标签以外部工具实现存在性作单向假阴性探测（zlint/pkilint/certlint/x509lint 任一实现即必可 lint），未对全部 389 条穷尽人工审计，是一处诚实的开放点。

**有效性威胁。** 内部：上游 IR 误差会沿 $\phi_C/\phi_G/\phi_V$ 传播并伪装成生成端问题（§8.5–§8.6），$\rho_R$ 为之提供可证伪修复窗口。外部：实证仅 RFC 5280 与 CABF BR、面向 zlint，跨框架非即插即用，已验证规则相对完整规范仍属子集。构念：编译通过只衡量可接受性而非语义忠实，对齐得分权重为经验值；为防"自己覆盖自己"，生成 lint 以 `cicasgen_*` 前缀与 zlint 原生两端互拒；证书级 oracle 受三边界约束（仅可认证子集约 254/389、为受控 fixture 经验测试而非定理、与 lint 端共享 `util.*` OID 表故盲于 OID 身份层错误），其外产出诚实归入"未验证"。
