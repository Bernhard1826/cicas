...
obligation, who is obligated, when the obligation applies, and what type of rule
...
explicit rather than locating the decision in an opaque end-to-end LLM call.
...
the IR rather than predicted by the LLM. For a fixed IR record, this Boolean
function yields the same label bit-for-bit on every run. The Layer~2 field
assignments used for a label are available for inspection.
