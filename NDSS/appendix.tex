...
The whole pipeline $\Pi$ chains four modules---extraction $\to$ lintability
determination $\to$ generation $\to$ verification:
\begin{equation}\label{eq:pipeline}
  \Pi = \phi_V \circ \phi_G \circ \phi_L \circ \phi_R
\end{equation}
where $\phi_R$ is extraction, $\phi_L$ is the four-condition lintability determination
of \S\ref{sec:lintability}, $\phi_G$ is the generation module restricted to the
DSL code space $\Tv$ of \S\ref{sec:codegen}, and $\phi_V$ is the three-layer semantic alignment of
\S\ref{sec:codegen}. $\phi_L$ is the
single-normative-rule lintability predicate $r \mapsto \{0,1\}$; the $\psi_L$ of
Algorithm~\ref{alg:saiv} is its set-level lift, which strips the noise $\RN$ from
$\Rkw$ and sorts the remaining candidates retained as normative rules into lintable $\RL$ and not-lintable
$\RU$, giving the partition $(\RN, \RL, \RU)$. The loop is driven by the
computable signals these modules produce.
...
on the lintability determination $\phi_L$ as repair-routing signals rather than
...
single consistency test on $\phi_L$, S1 auditing the not-lintable verdict and S2
...
If the lintability partition $\psi_L$ assigns each keyword-recalled candidate in
...
such hit is adjudicated as either a $\phi_L$ false negative and re-extracted to
...
normative rule after generation repair $\mathcal{P}_G$ and vocabulary expansion
$\mathcal{P}_A$ have both been exhausted is a candidate signal that $\phi_L$ mislabeled it.
...
$\phi_L$ false positive and re-derived to not-lintable, or a genuine $\Tv$
...
\S\ref{subsec:repair}.
\textsc{RouteRepair} in Algorithm~\ref{alg:saiv} denotes the operator-guided
routing step that uses the residuals, lintability-check signals, and failure
traces to select the next applicable repair direction from
$\{\mathcal{P}_R,\mathcal{P}_L,\mathcal{P}_G,\mathcal{P}_V\}$.

\subsection{Repair Operations}
\label{subsec:repair}

SAIV comprises four kinds of operations---IR-content
repair $\mathcal{P}_R$, lintability repair $\mathcal{P}_L$, generation repair
$\mathcal{P}_G$, and verification repair $\mathcal{P}_V$. The repair operator
$\mathcal{P}$ is distinct
from the rendering function $\rho$ of \S\ref{subsec:mech}. The next round's
operation is selected from the signals above, and a repair is accepted only
when the metric decreases.

\begin{itemize}
\item \textbf{IR-content repair $\mathcal{P}_R$} changes the extraction prompt
...
\item \textbf{Lintability repair $\mathcal{P}_L$} is the same re-extraction
  ...
  a certificate lint. After re-extraction, SAIV re-runs
  $\phi_L$ and updates the set-level partition $\psi_L$.
\item \textbf{Generation repair $\mathcal{P}_G$} routes by cause. A missing template routes to the
  offline vocabulary-expansion channel $\mathcal{P}_A$, which adds a new atomic
  template to $\mathcal{A}$ after certificate-level execution verification; a wrong IR field, such as too coarse a granularity,
  routes to IR-content repair $\mathcal{P}_R$; and a normative rule that proves not to be lintable
  routes to lintability repair $\mathcal{P}_L$. When the template, IR, and
  ...
  repaired as code.
\item \textbf{Verification repair $\mathcal{P}_V$} enlarges the semantic-equivalence
