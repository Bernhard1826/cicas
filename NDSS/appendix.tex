...
general PKI certificate-attribute judgment reusable across multiple rules and is
...
auxiliary checks audit the lintability decision without contributing to the
objective loss; when a lintability label is suspected to be wrong, SAIV updates
the relevant IR fields and applies the lintability decision again in the next
iteration.
...
\end{equation}
where $\phi_R$ is extraction, $\phi_L$ is the four-condition lintability decision
...
single-rule lintability predicate $r \mapsto \{0,1\}$; the $\psi_L$ of
...
on the lintability decision $\phi_L$ as repair-routing signals rather than
separate objective terms. Because these checks can change which rules enter
$\RL$ and $\RLunc$, they can affect the synonymy residual in later iterations
...
RFC~2119 keyword matches rejected as non-rules by the adjudication procedure,
$\RL$ the lintable rules, and $\RU$
the not-lintable rules. The lintable class $\RL$ is further split by ``whether
...
\medskip
\noindent\textbf{S1: reverse lintability of covered rules.} The first
auxiliary check audits the not-lintable verdict. If a rule has a corresponding
...
audits the lintable verdict. A rule judged lintable should be synthesizable
...
G2 Synonymy & $\Lcode$, Eq.~\eqref{eq:code-res} &
  ...
  re-check \\
\midrule
\multicolumn{4}{@{}l}{\emph{Auxiliary lintability checks --- police $\phi_L$,
  outside $\Ltot$}}\\
...
\item \textbf{Lintability repair $\mathcal{P}_L$}: is the same re-extraction
  ...
  S1 indicates that a not-lintable rule is actually covered by an existing lint,
  or when S2 indicates that a supposedly lintable rule may not be expressible as
  a single-certificate static check. After re-extraction, SAIV re-runs
...
\item \textbf{Generation repair $\mathcal{P}_G$}: A missing template routes to the
  ...
  routes to IR-content repair $\mathcal{P}_R$; and a rule that proves not to be lintable
  ...
  within $\Tv$, the rule is kept as a logged expressiveness gap rather than
  repaired as code.
