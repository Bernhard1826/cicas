...
the whole pipeline is trained to approach, and treats the two auxiliary checks
...
counts the violations $\Nviol$ and is clear when $\Nviol = 0$. $\Nviol$ is
...
$\mathcal{P}_A$ are exhausted is a candidate signal that $\phi_L$ mislabeled it.
...
\item \textbf{Generation repair $\mathcal{P}_G$}: A missing template routes to the
  ...
  routes to IR-content repair $\mathcal{P}_R$; and a rule that proves not lintable
...
\textbf{Termination conditions}: $\Ltot < \theta$ and the two
lintability checks clear, reaching the maximum iteration cap $K$, or no residual
