...
To limit the role of the LLM in extraction, we design DLEP as a dual-layer pipeline that separates deterministic candidate recall from schema-constrained semantic parsing.
...
Second, the IR serves as the fixed input to the four-condition lintability analysis in \S\ref{sec:method-lintability}, making that analysis deterministic for a given IR.
Third, once a rule is determined to be lintable, field paths, assertion types, and constraint values are explicit inputs to DSL-tree synthesis and inspection.
