...
The coverage analysis also exposes a scoping issue: of the 31 lints in the
...
\item \textbf{(i) Method expressiveness.} The synonymy-judgment endpoint is still
  implemented by an LLM, and the 79 atomic templates still have gaps for
...
\item \textbf{(iv) Coverage and generalization boundary.} The lintable total 336
  and the codegen target 204 are different scopes, and for cross-ecosystem
...
\item \textbf{(v) End-to-end scope.} ``Code $\equiv$ specification'' holds only
  ...
  the lintability label has not been exhaustively manually audited over all 336
  rules.
