...
LLM response. Instead, it confines a normative rule's code body to a finite, closed
...
tree and pass ``render and go build'' before it enters semantic verification.
...
IR provides only ``which field this normative rule actually constrains, and what value,''
...
the cross-modal judgment ``does the code implement the
normative rule'' into the same-modal judgment ``are two natural-language
sentences semantically equivalent.'' Following $\smech$ of \S\ref{subsec:mech}, with the
