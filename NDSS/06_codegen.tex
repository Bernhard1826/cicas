...
Our code-generation design does not emit Go code directly from an unrestricted
LLM response. Instead, it confines a rule's code body to a finite, closed
DSL-tree space $\Tv$. Each accepted code body is a tree in that space, which a
deterministic function renders into Go code. The generator first attempts
deterministic synthesis. When that path yields no tree, the LLM fallback may
propose a tree from the same closed space. Its output must parse as an in-space
tree and pass ``render and go build'' before it can be shipped.
...
structural checks, and neither decides whether the code expresses the
...
Whether the generated code expresses the specification is the hardest judgment
...
i.e., $\mathrm{Code}\equiv t$; and because $t$ is deterministically reduced from
the IR, \CodeEqIR.
...
representation IR, which is not the same as code--specification
alignment; if the IR itself has already drifted from the specification, this
verification cannot detect it. The code--specification decision therefore still
rests with \CodeEqSpec{}, the Layer~B synonymy judgment.
