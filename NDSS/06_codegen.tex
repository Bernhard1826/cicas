...
tree, render, and pass go build before it enters semantic verification.
...
the placeholder for an absent precondition, a normal value distinct from the $\bot$
that denotes synthesis failure in the table of \S\ref{subsec:synth}. In the
...
This subsection gives the implementation of $\phi_G$. The generator
first attempts deterministic synthesis and triggers LLM synthesis only when it
returns $\bot$. Here $\bot$ denotes synthesis failure, meaning no result at all,
and is distinct
from the normal value $\varepsilon$, which denotes an absent precondition in
\S\ref{subsec:atoms}. Both paths use rendering and passing go build as
...
synthesis-failure $\bot$ report that no usable result was produced, so they share the $\bot$
form, whereas $\varepsilon$ denotes the normal case of an absent precondition. The raw LLM
