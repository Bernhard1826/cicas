...
\Repeat
  \State compute objective residuals $\Lrec$~\eqref{eq:recall-res},
    $\Lcode$~\eqref{eq:code-res}; lintability-check signals $\Nviol$ and $\bot_{\mathrm{NT}}$
  \If{$\Ltot<\theta$ \textbf{and} $\Nviol=0$}
    \State \textbf{break} \Comment{objectives converged, checks clear}
  \EndIf
  \State $\mathit{op}\gets \textsc{RouteRepair}(\Lrec,\Lcode,\Nviol,\bot_{\mathrm{NT}},\cdots)$
    \Comment{repair routing, \S\ref{subsec:repair}}
