...
Stage-Aware Iterative Verification, abbreviated SAIV; an automatic gate handles in-loop
acceptance.
\par\smallskip
\small
\begin{algorithmic}[1]
...
\Repeat
  ...
  \State $\mathit{stage}\gets \textsc{StageAttribution}(\cdots)$
    \Comment{operator routing, \S\ref{subsec:repair}}
  ...
  \State $t\gets t+1$
\Until{$t\ge K$ \textbf{or} no residual decreased, the loop-until-dry condition}
