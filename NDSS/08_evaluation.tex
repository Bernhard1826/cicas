...
\S\ref{subsec:coverage}), it provides \textbf{two
mutually independent external validations}, each checking one half of the
framework: \S\ref{subsec:lintability-ext} externally validates \textbf{rule
extraction and lintability determination} against the zlint maintainers' manual
gold standard, and \S\ref{subsec:certdetect} externally validates \textbf{code
...
\textbf{lintable (single-cert observable)} & \textbf{248} &
...
zlint coverage (full; existing cognate lint) & 158 &
  the rule is fully implemented by some cognate zlint certificate lint; a CABF
  rule may be covered by an inherited RFC~5280 lint \\
\textbf{uncovered lintable (codegen domain)} & \textbf{90} &
  needs the system to generate its own lint ($248 - 158$); CABF 64 $+$ RFC~5280
  26, the target set of downstream code generation $\phi_G$ \\
\textbf{can generate compilable lint} & \textbf{90} &
  deterministic path 87 $+$ LLM fallback path 3, both gated by ``render and
  pass go build''; \textbf{generation rate $= 90/90 = 100.0\%$} \\
cannot generate (no tree / not compiling / abstain) & 0 &
  no generation failure in the current codegen domain \\
\textbf{final-shipping strict synonymous (\CodeEqSpec)} & \textbf{90} &
  ...
  \textbf{strict synonymy rate $= 90/90 = 100.0\%$} \\
final-shipping not synonymous (DNE) & 0 &
...
Appendix~\ref{subsec:targets}): the total number of keyword-recalled rules should
...
\textbf{90}---the domain of downstream code generation $\phi_G$.
...
Algorithm~\ref{alg:coverage} formalizes this determination: summarize a lint
...
Of the 248 lintable rules, \textbf{zlint fully covers 158 and leaves 90
uncovered} (Table~\ref{tab:coverage}); the 90 uncovered are the domain of code
generation $\phi_G$.
...
full (full coverage) & 106 & 52 & \textbf{158} \\
uncovered (codegen domain) & 64 & 26 & \textbf{90} \\
lintable total & 170 & 78 & \textbf{248} \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
Version & Sheet rows (Yes/No) & \textbf{Match cov.} & Agreement &
  \textbf{Cohen's $\kappa$} & F1 \\
\midrule
\textbf{BR~1.4.8} & 122 (75/47) & \textbf{86.9\%} & 91.5\% & \textbf{0.823} & 0.930 \\
\textbf{BR~2.0.2} & 250 (17/233) & \textbf{86.8\%} & 96.8\% & 0.616 & 0.632 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{This external validation checks the first half of the system.} On
...
cicasgen\_ hits, of which \textbf{299} are independently confirmed (251 Error $+$
